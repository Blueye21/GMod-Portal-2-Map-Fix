print("[P2] glados_scenetable_include_manual.lua loaded")
--[[
Properties:
...
// ====================================== Turrets deployed scene
		...
		*/
		...
		{
			printldebug("==Starting wheatley neurotoxin ride Nag")
			GladosPlayVcd( -750 )
		}
		
		function StopWheatleyNeurotoxinRideNag()
		{
			printldebug("==Stopping wheatley neurotoxin ride Nag")
			GladosStopNag()
		...
		{
			printldebug("==Starting wheatley plugin Nag")
			GladosPlayVcd( -761 )
		}
		
		function StopWheatleyPluginNag()
		{
			printldebug("==Stopping wheatley plugin Nag")			
...
// JAYP ADD IN BUTTON COUGHS		
		...
		{
			printldebug("==Starting stalemate button Nag")
			GladosPlayVcd( -755 )
		}
		
		function StopStalemateButtonPressNag()
		{
			printldebug("==Stopping stalemate button Nag")
			nuke() // stop every scene
		}
...
// JAY -- Talked to Wolpaw, he said lets just keep the gibberish off.  So please check with him before re-enabling the gibberish.
		...
		{
			printldebug("==Starting Glados Nag")
			GladosPlayVcd( -93 )
		}
		
		function StopGladosGibberishNag()
		{
			printldebug("==Stopping Glados Nag")
			GladosStopNag()
		...
		{
			if( !CorePlayerInElevator )
			{
				printldebug("==Starting wheatley Nag")
				GladosPlayVcd( -666 )
			}
			else
			{
				printldebug("==SKIPPING wheatley Nag -- player already in elevator")
			}
		}
		
		function StopWheatleyElevatorNag()
		{
			printldebug("==Stopping wheatley Nag")
			nuke()
		}
...
{
		if (!sp_a2_pit_flings_future_starter)
		{
...
function sp_laser_lift_pit_flings_cube_lost()
{
		...
		switch ( boxDissolveCount )
		{
			...
			case 2:
				...
				break
			default:
				printldebug("====== Cube lost! spawning a new one...")
				EntFire( "drop_new_box_relay", "trigger", 0, 0, 0 )
				break
		}
		boxDissolveCount+=1
...
{
		printldebug("***DISSOLVING cube_dropper_box!")
...
{
	printldebug("I should be triggering sp_laser_over_gooElevatorStop01")
	printldebug("The current function that does this is sp_a2_laser_over_goo_elevator_stoppage() in glados.nut")
	GladosPlayVcd(465)
}
...
{
	printldebug("=====================***************************************CALLING WINDOW SCENE")
	GladosPlayVcd( -501 )
}
...
if curMapName=="sp_a1_intro11"  then
	///////////////////////////////////////////////
	///////////////////////////////////////////////
...
if (curMapName=="sp_a2_core" )
{
	...
	for ( local i=1; i < 25; i++ )
	{
		...
		while( previous_random_anim == random_anim)
		{
			random_anim = RandomInt(1,4)
		}
