...
function GrantGameCompleteAchievementHUB()
	...
	for i = 1, 5 do
		if  IsPlayerBranchComplete( 0, i ) == false then
			...
			--print("!!=== bGameCompleteB = false --- not completed branch " + i )
		end
	...
	if bGameCompleteB then
		...
		--print("!!!!!!!!=== AWARDING END GAME ACHIEVEMENT!!!" )
	end
...
function EarnTaunt( nTaunt )
	...
	if bGrantTauntB == false && bGrantTauntO == false then
...
function TransitionToCredits()
	if IsLocalSplitScreen() then
		...
		timer.Simple(0.1, function(
			RunConsoleCommand("changelevel", "mp_coop_credits")
		))
	else
